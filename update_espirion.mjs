...
const files = fs.readdirSync(srcFolder)
  .filter(f => f.startsWith('media__17752172') && f.endsWith('.jpg'))
  .sort((a, b) => a.localeCompare(b));

if (files.length >= 5) {
  for (let i = 0; i < 5; i++) {
    fs.copyFileSync(path.join(srcFolder, files[i]), path.join(destFolder, `espirion_new_${i+1}.jpg`));
    console.log(`Copied ${files[i]} to espirion_new_${i+1}.jpg`);
  }
} else {
