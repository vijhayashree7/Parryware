...
const images = fs.readdirSync(srcFolder).filter(f => f.startsWith('media__') && (f.endsWith('.png') || f.endsWith('.jpg')));
images.sort();
const latest5 = images.slice(-5);

const mappings = [
  { img: latest5[1], name: 'praseo_item_1' }, // Praseo Chrome
  { img: latest5[2], name: 'praseo_item_2' }, // Quattro Geometric
  { img: latest5[4], name: 'praseo_item_3' }, // Elara Warm Metallic
  { img: latest5[3], name: 'praseo_item_4' }, // Nero Minimal
  { img: latest5[0], name: 'praseo_item_5' }  // Aurum Gold
];
...
const products = [
  ...
  {
    id: 4,
    name: 'Nero Minimal Styled Faucet',
    description: 'A bold minimal silhouette, designed for modern bathrooms with a strong visual identity.',
    price: 3500,
    mrp: 6000,
    image: '${getPath(3)}',
    rating: 4.8,
...
console.log('Praseo recompiled exactly to 5 unique slots!');
