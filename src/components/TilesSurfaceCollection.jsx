...
export default function TilesSurfaceCollection() {
  ...
  const handleAddToCart = (product) => {
    if (!cart.find(item => item.product.id === product.id)) {
      setCart([...cart, { product, quantity: 100 }]); // Add minimum 100 sqft for tiles
    }
  };

  const handleIncrease = (productId) => {
    setCart(cart.map(item => 
      item.product.id === productId ? { ...item, quantity: item.quantity + 20 } : item // increment by 20 sqft
    ));
  };
  ...
  const handleDecrease = (productId) => {
    setCart(cart.map(item => {
      if (item.product.id === productId) {
        if (item.quantity <= 20) return null; // Remove if less than 20
        return { ...item, quantity: item.quantity - 20 };
      }
      return item;
    }).filter(Boolean));
  };
        ...
        <div className="relative flex items-center cursor-pointer p-2 rounded-full hover:bg-gray-100 transition-colors">
          ...
          {cart.length > 0 && (
            <span className="absolute top-0 right-0 bg-[#E33939] text-white font-bold text-[10px] rounded-full w-[18px] h-[18px] flex items-center justify-center shadow-md">
              {cart.reduce((total, item) => total + 1, 0)} {/* Cart item count */}
            </span>
          )}
        </div>
      </header>
                  ...
                  <div className="mt-auto">
                    {cartItem ? (
                      <div className="w-full h-[52px] flex items-center justify-between border-[1px] border-gray-800 rounded-full overflow-hidden bg-white text-gray-900 shadow-sm">
                        <button 
                          onClick={() => cartItem.quantity <= 20 ? handleRemoveFromCart(product.id) : handleDecrease(product.id)} 
